from __future__ import annotations

from pathlib import Path
from typing import Any

from accelerate import PartialState
from peft import PeftModel
from trl import GRPOConfig, GRPOTrainer

from .cli_config import GRPOJobConfig, resolve_resume
from .devices import clear_device_cache, detect_runtime
from .hub import require_hf_namespace
from .judge import configure_scientific_design_judge, scientific_design_reward
from .modeling import load_causal_lm, load_tokenizer
from .teacher import require_openai_key
from .train_common import (
    configure_environment,
    hf_token,
    load_training_dataset,
    require_dataset_schema,
    run_manifest,
    smoke_subset,
    write_json,
    write_jsonl,
)
from .versions import require_training_stack

GRPO_RUBRIC_COLUMNS = {
    "prompt",
    "task",
    "required_constraints",
    "evaluation_criteria",
    "acceptable_alternatives",
    "failure_modes",
}


def run_grpo(config: GRPOJobConfig, *, smoke_test: bool = False) -> dict[str, Any]:
    configure_environment(config.runtime)
    require_openai_key()
    versions = require_training_stack()
    runtime = detect_runtime()
    token = hf_token()
    output_dir = Path(config.output.directory).expanduser()
    resume_from_checkpoint = resolve_resume(output_dir, config.output.resume)

    if PartialState().num_processes != 1:
        raise RuntimeError(
            "The live API-judged GRPO command requires one training process "
            "(WORLD_SIZE=1)."
        )
    if config.hub.push:
        require_hf_namespace(config.hub.repo_id, token=token)

    dataset = load_training_dataset(config.data, token=token)
    require_dataset_schema(
        dataset,
        splits=("train", "validation"),
        columns=GRPO_RUBRIC_COLUMNS,
    )
    if smoke_test:
        dataset = smoke_subset(dataset)

    adapter_source = config.adapter.selected_source
    if config.adapter.source == "local" and not Path(adapter_source).expanduser().exists():
        raise FileNotFoundError(f"Local SFT adapter does not exist: {adapter_source}")

    tokenizer = load_tokenizer(config.model.model_id, token=token)
    clear_device_cache(runtime)
    base_model = load_causal_lm(config.model.model_id, runtime, token=token)
    model = PeftModel.from_pretrained(
        base_model,
        adapter_source,
        is_trainable=True,
        token=token,
    )
    reward = config.reward
    configure_scientific_design_judge(
        model=reward.judge_model,
        cache_path=reward.cache_path,
        format_base_reward=reward.format_base_reward,
        semantic_reward_weight=reward.semantic_reward_weight,
        api_max_retries=reward.openai_max_retries,
        api_timeout_seconds=reward.openai_timeout_seconds,
    )
    training = config.training
    pin_memory = (
        runtime.backend == "cuda"
        if config.runtime.pin_memory_on_cuda_only
        else True
    )
    args = GRPOConfig(
        output_dir=str(output_dir),
        num_train_epochs=training.num_train_epochs,
        max_steps=training.max_steps,
        learning_rate=training.learning_rate,
        per_device_train_batch_size=training.train_batch_size,
        per_device_eval_batch_size=training.eval_batch_size,
        gradient_accumulation_steps=training.gradient_accumulation_steps,
        num_generations=training.num_generations,
        num_generations_eval=training.num_generations_eval,
        max_completion_length=training.max_completion_length,
        temperature=training.temperature,
        top_p=training.top_p,
        top_k=training.top_k,
        beta=training.beta,
        reward_weights=list(reward.function_weights),
        scale_rewards=training.scale_rewards,
        multi_objective_aggregation=training.multi_objective_aggregation,
        loss_type=training.loss_type,
        remove_unused_columns=False,
        chat_template_kwargs={"enable_thinking": False},
        gradient_checkpointing=training.gradient_checkpointing,
        gradient_checkpointing_kwargs={
            "use_reentrant": training.gradient_checkpointing_use_reentrant
        },
        optim=training.optimizer,
        warmup_steps=training.warmup_steps,
        eval_strategy=training.eval_strategy,
        eval_steps=training.eval_steps,
        save_strategy=training.save_strategy,
        save_steps=training.save_steps,
        save_total_limit=training.save_total_limit,
        logging_steps=training.logging_steps,
        logging_first_step=training.logging_first_step,
        log_completions=training.log_completions,
        num_completions_to_print=training.num_completions_to_print,
        report_to=training.report_to,
        dataloader_pin_memory=pin_memory,
        push_to_hub=config.hub.push,
        hub_model_id=config.hub.repo_id,
        hub_strategy=config.hub.strategy,
        hub_private_repo=config.hub.private,
        hub_token=token,
        hub_always_push=config.hub.always_push,
        bf16=runtime.trainer_bf16,
        fp16=runtime.trainer_fp16,
        use_cpu=runtime.use_cpu,
        use_vllm=training.use_vllm,
        seed=training.seed,
    )
    trainer = GRPOTrainer(
        model=model,
        args=args,
        reward_funcs=[scientific_design_reward],
        train_dataset=dataset["train"],
        eval_dataset=dataset["validation"],
        processing_class=tokenizer,
    )
    output_dir.mkdir(parents=True, exist_ok=True)
    write_json(
        output_dir / "resolved_config.json",
        run_manifest(
            stage="grpo",
            config=config,
            runtime=runtime,
            versions=versions,
            resume_from_checkpoint=resume_from_checkpoint,
            smoke_test=smoke_test,
        ),
    )
    train_result = trainer.train(resume_from_checkpoint=resume_from_checkpoint)
    trainer.save_model(output_dir)
    tokenizer.save_pretrained(output_dir)
    if config.hub.push:
        trainer.push_to_hub(
            commit_message="Complete scientific design GRPO CLI training"
        )
    write_json(output_dir / "train_metrics.json", train_result.metrics)
    write_jsonl(output_dir / "log_history.jsonl", trainer.state.log_history)
    return {
        "output_directory": str(output_dir),
        "hub_repository": config.hub.repo_id if config.hub.push else None,
        "metrics": train_result.metrics,
    }
